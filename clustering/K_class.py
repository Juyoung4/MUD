...
class call_Dataset:
    ...
    def __init__(self):
        ...
        try:
            print('DB연결 성공')
            rawDatasetjson = requests.get(url="http://34.84.147.192:8000/news/articles/?limit=30000")
            if rawDatasetjson.status_code == 200:
                rawDatasetjson =rawDatasetjson.json()
                print(rawDatasetjson['count'])
...
class store_Clusterid:
    def __init__(self,finalData,rawDataresult,labelsize,category):
    ...
    def creatCluster(self,clusterHeadline, clusterSummary,labelsize,category):
        import requests
    ...
    def replace_clusterid_to_uuid(self):
        ...
        for i in range (len(labelset)):
            cluster_uuid_list.append(self.creatCluster(newClusterHeadline,newClusterSummary,self.labelsize[i],self.category))
...
class run_kmeans:
    ...
    def play(self):
        ...
        s = store_Clusterid(Final_Data,self.rawDataresult,labelsize,self.category)
        s.store()
    ...
    def play_fixK(self):
        ...
        # o = optimal_K(preprocessed_Data, self.sK, self.eK)
        # k = o.final_find_K()  # 최적의 K
        finalkmeans = KMeans(n_clusters=20, init='k-means++', n_init=600).fit(preprocessed_Data)#600

        label =finalkmeans.labels_
        # labelsize ={}
        # for i in set(label):
        #     labelsize[i] = list(label).count(i)
        # a =list(labelsize.values()).sort()
        # for idx,values in enumerate(a):
        #     for j in lab
        a=collections.Counter(label)
        b=list(a.values())
        b.sort(reverse=False)
        for k,v in a.items():
            a[k] = b.index(v)
        labelsize=list(a.values())
