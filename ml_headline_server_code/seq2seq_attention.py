...
title,content = util.loading_data(datapath,eng=False, num=False, punc=False)
#title,content = util3.normalize(datapath)
word_to_ix,ix_to_word = util.make_dict(title + content, minlength=0, maxlength=3,jamo_delete=True)
...
batch_size = 16
encoder_size = 100
decoder_size = util.doclength(title, sep=True)
#decoder_size = util3.doclength(title, sep=True) # (Maximum) number of time steps in this batch
steps_per_checkpoint = 10

# transform data
encoderinputs, decoderinputs, targets_, targetweights = util.make_suffle(content, title, word_to_ix, encoder_size=encoder_size, decoder_size=decoder_size, shuffle=False)

class seq2seq_attetion(object):
...
model = seq2seq_attetion(multi=multi, hidden_size=hidden_size, num_layers=num_layers,
...
while current_step < 10000001:
    ...
    encoder_inputs, decoder_inputs, targets, target_weights = util.make_batch(encoderinputs[start:end],decoderinputs[start:end],targets_[start:end],targetweights[start:end])
