...
Headline과 <br>Content 추출 | 실제 뉴스 기사를 직접 읽게 하지 않고, 군집화된 토픽들에서 (유사 기사내용이 담긴 뉴스들 묶음) HeadLine 제목을 추출, 군집 낸 기사내용전체를 요약하여 간단하게 Content로 보여줌  | 추상적 요약 + 추상적 요약
개인 추천 시스템 |  |추천 알고리즘
...
- K-mean 클러스터링
  ...
  - K-means는 비지도 학습이고 라벨링되지 않은 데이터 집합을 클로스터로 분할합니다. 분할된 클러스터 내의 점들 사이의 거리를 가깝게 만듭니다.
